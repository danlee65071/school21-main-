...
static size_t	proc_int_minus(t_flags flags, int buf, char *str, int n)
{
	size_t	len;

	len = 0;
	if (buf < 0 && flags.precision >= 0 && n > -2147483648)
		write(1, "-", 1);
...
static void	sup_func(t_flags *flags, int *n, size_t *len)
{
...
size_t	proc_int(int n, t_flags flags)
{
	...
	else
	{
		if (n < 0 && (flags.precision >= 0 || flags.zero == 1))
			sup_func(&flags, &n, &len);
		str = ft_itoa(n);
		if (flags.minus == 1)
			len += proc_int_minus(flags, buf, str, n);
